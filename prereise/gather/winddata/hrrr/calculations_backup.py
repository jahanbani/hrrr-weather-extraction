            ...
            try:
                ...
                with pygrib.open(group['f00']) as grbs:
                    ...
                    if not variable_found:
                        print(f"Variable {var_selector} not found in {group['f00']}")
...
def calculate_safe_worker_count(available_memory_gb, chunk_size, num_variables):
    ...
    # For 256GB system, use only 20% of available memory to be safe
    # This prevents any chance of memory exhaustion
    safe_memory_mb = available_memory_gb * 1024 * 0.20  # 20% of available memory
    
    max_workers = int(safe_memory_mb / memory_per_worker_mb)
    
    # Ensure reasonable bounds - be conservative based on actual usage
    max_workers = max(1, min(max_workers, 4))  # Cap at 4 workers for safety
    
    print(f"Memory calculation:")
    print(f"  Available memory: {available_memory_gb:.1f} GB")
    print(f"  Safe memory limit: {safe_memory_mb:.1f} MB (20% of available)")
...
def monitor_memory_usage(warning_threshold_gb=20):
    ...
    try:
        ...
        if used_gb > warning_threshold_gb:
            ...
            print(f"   Consider reducing workers or chunk size")
            
            # If memory usage is very high (>85%), recommend even more conservative settings
            if percent_used > 85:
...
def check_existing_outputs(output_dir, var_name, date_range):
    ...
    if os.path.exists(var_output_dir):
        ...
        for item in os.listdir(var_output_dir):
            item_path = os.path.join(var_output_dir, item)
            if os.path.isdir(item_path) and item.isdigit() and len(item) == 8:
                # Convert YYYYMMDD to datetime
                try:
                    date = datetime.datetime.strptime(item, '%Y%m%d').date()
                    if date in date_range:
                        # Check if this date directory has any chunk files
                        chunk_files = [f for f in os.listdir(item_path) if f.endswith('.parquet')]
                        if chunk_files:
    ...
    for day_idx, current_date in enumerate(remaining_dates, 1):
        ...
        try:
            ...
            if day_result and day_result.get("status") == "completed":
                ...
                print(f"   ✅ Completed in {day_time:.1f}s")
            else:
                failed_days.append(current_date.date())
                print(f"   ❌ Failed after {day_time:.1f}s")
...
def get_optimized_settings_for_high_performance_system():
    ...
    if cpu_count >= 32 and total_memory_gb >= 200:
        # High-performance system (36+ CPUs, 256GB+ RAM)
        print("🚀 Using HIGH-PERFORMANCE settings")
        
        # Aggressive settings for day-by-day processing
        settings = {
            'chunk_size': 500000,        # Large chunks (5x larger)
            'num_cpu_workers': 24,       # Use 2/3 of CPUs (24/36)
            'num_io_workers': 8,         # More I/O workers
            'max_file_groups': 15000,    # More file groups
            'queue_maxsize': 48,         # Larger queue
            'batch_size_multiplier': 3,  # Process more batches
...
def get_aggressive_parallel_settings():
    ...
    if settings['num_cpu_workers'] >= 20:
        # Very aggressive settings for high-performance systems
        # BUT with smaller chunks for better pyarrow compatibility
        aggressive_settings = {
            'chunk_size': 75000,         # SMALLER chunks for better pyarrow compatibility
            'num_cpu_workers': 32,       # Use almost all CPUs
            'num_io_workers': 12,        # More I/O workers
            'max_file_groups': 20000,    # More file groups
            'queue_maxsize': 64,         # Larger queue
            'batch_size_multiplier': 4,  # Process many batches
            'memory_safety_factor': 0.5, # Use 50% of memory
            'compression': 'snappy',
            'parallel_file_writing': True,
            'enable_resume': True,
            'day_output_dir_format': 'flat',
        ...
        print("🔥 Using AGGRESSIVE parallel settings for day-by-day processing")
        print(f"   Chunk size: {aggressive_settings['chunk_size']:,} (smaller for pyarrow compatibility)")
...
def extract_all_variables_from_grib_file(file_path, var_selectors, chunk_indices, grid_lats, grid_lons):
    ...
    try:
        with pygrib.open(file_path) as grbs:
            ...
            for grb in grbs:
                ...
                for var_name, var_selector in var_selectors.items():
                    if var_name in found_variables:
                        continue  # Already found this variable
                    ...
                    if grb_name == var_selector:
                        ...
                        if values is not None:
                            ...
                            if nan_count > 0:
                                print(f"  🔴 WARNING: Found {nan_count} NaN values in {var_name} at {timestamp}")
                                print(f"     File: {os.path.basename(file_path)}")
                                print(f"     Grid points with NaN: {np.where(np.isnan(values))[0]}")
                                
                                # Log to file for detailed analysis
                                nan_log_file = os.path.join(os.path.dirname(file_path), "nan_detection_extraction.log")
                                with open(nan_log_file, 'a') as f:
                                    f.write(f"{datetime.datetime.now().isoformat()} | {var_name} | {timestamp} | {os.path.basename(file_path)} | NaNs: {nan_count}\n")
                            
                            # Create numbered column names for this chunk
                            chunk_columns = [f"grid_{i:06d}" for i in range(len(chunk_indices))]
                            results[var_name][timestamp] = dict(zip(chunk_columns, values))
                            found_variables.add(var_name)
                            
                            print(f"  Found {var_name} in {os.path.basename(file_path)}")
                        
                        # If we've found all variables, we can stop early
            ...
            missing_vars = set(var_selectors.keys()) - found_variables
            if missing_vars:
