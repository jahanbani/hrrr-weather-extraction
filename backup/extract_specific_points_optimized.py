#!/usr/bin/env python3
"""
Optimized function to extract HRRR data for specific wind and solar locations.
This function reads wind and solar points from CSV files and extracts only the 
closest grid points for those specific locations.
"""
...
def formatted_filename(dt, hours_forecasted="0"):
    """Format filename for HRRR GRIB files.
    
    Args:
        dt (datetime): Date and time
        hours_forecasted (str): Forecast hour
        
    Returns:
        str: Formatted filename
    """
    return f"hrrr.t{dt.hour:02d}z.wrfsubhf{hours_forecasted}.grib2"
...
def process_grib_file_for_points(file_path, var_selector, wind_indices, solar_indices, 
                               grid_lats, grid_lons, wind_points, solar_points):
    """Process a single GRIB file and extract data for wind and solar points.
    
    Args:
        file_path (str): Path to GRIB file
        var_selector (str): Variable selector name
        wind_indices (np.array): Wind point indices
        solar_indices (np.array): Solar point indices
        grid_lats (np.array): Grid latitudes
        grid_lons (np.array): Grid longitudes
        wind_points (pd.DataFrame): Wind points DataFrame
        solar_points (pd.DataFrame): Solar points DataFrame
        
    Returns:
        dict: Dictionary with extracted data for wind and solar points
    """
    file_data = {
        'wind': {},
        'solar': {}
        ...
        with pygrib.open(file_path) as grbs:
            for grb in grbs:
                if grb.name == var_selector:
                    ...
                    # Extract values for wind points
                    if len(wind_indices) > 0:
                        ...
                        if wind_values is not None:
                            wind_columns = [f"wind_{pid}" for pid in wind_points.pid.astype(str)]
                            file_data['wind'][timestamp] = dict(zip(wind_columns, wind_values))
                    
                    # Extract values for solar points
                    if len(solar_indices) > 0:
                        ...
                        if solar_values is not None:
                            solar_columns = [f"solar_{pid}" for pid in solar_points.pid.astype(str)]
                            file_data['solar'][timestamp] = dict(zip(solar_columns, solar_values))
...
def extract_specific_points_optimized(
    wind_csv_path,
    solar_csv_path,
    START,
    END,
    ...
    """
    Extract HRRR data for specific wind and solar locations with optimized processing.
    
    This function reads wind and solar points from CSV files and extracts only the
    closest grid points for those specific locations, significantly reducing processing time
    and memory usage compared to full grid extraction.
    ...
    print("🚀 OPTIMIZED SPECIFIC POINTS EXTRACTION")
    print("=" * 60)
    ...
    if num_workers is None:
        ...
        num_workers = os.cpu_count()  # Use all CPUs
    
    if max_file_groups is None:
        max_file_groups = 50000  # Increased for better throughput
    
    print(f"🎯 Using settings:")
    print(f"   Workers: {num_workers} (using all {os.cpu_count()} CPUs)")
    ...
    # Phase 8: Process each file group (open GRIB file once per group)
    print("🚀 Processing file groups...")
    ...
    all_solar_data = defaultdict(dict)
        ...
        # Process each file group in parallel (open GRIB file once per group)
    from concurrent.futures import ThreadPoolExecutor, as_completed
    import threading
    
    # Thread-safe data storage
    wind_data_lock = threading.Lock()
    solar_data_lock = threading.Lock()
    
    def process_file_group(key):
        """Process a single file group and return extracted data."""
    ...
    if use_parallel and num_workers > 1:
        ...
        print(f"   Processing {len(file_groups)} file groups with {num_workers} workers")
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            # Submit all file groups for parallel processing
            future_to_key = {
                executor.submit(process_file_group, key): key 
            ...
            for future in tqdm(as_completed(future_to_key), 
                ...
                try:
                    _, group_wind_data, group_solar_data = future.result()
                    ...
                    if completed_count % 10 == 0:  # Progress update every 10 files
                        ...
                        eta = (len(file_groups) - completed_count) / rate if rate > 0 else 0
                        print(f"   ⚡ Processed {completed_count}/{len(file_groups)} files ({rate:.1f} files/sec, ETA: {eta:.1f}s)")
                        if PSUTIL_AVAILABLE:
        ...
        print(f"   📊 Average rate: {len(file_groups)/parallel_time:.1f} files/sec")
    else:
        # Sequential processing (fallback)
        print(f"🔄 Using sequential processing (workers: {num_workers})...")
        
        for key in tqdm(sorted(file_groups.keys()), desc="Processing file groups"):
            try:
                _, group_wind_data, group_solar_data = process_file_group(key)
    ...
    if "UWind10" in all_solar_data and "VWind10" in all_solar_data:
        wind_speed_10 = np.sqrt(all_solar_data["UWind10"]**2 + all_solar_data["VWind10"]**2)
        # Keep the same column names as the original wind components
        wind_speed_10.columns = wind_speed_10.columns
        
        # Create variable-specific subfolder for derived wind speed in solar output directory
        var_subfolder = os.path.join(solar_output_dir, "WindSpeed10")
        os.makedirs(var_subfolder, exist_ok=True)
    ...
    print(f"Optimization: Each GRIB file opened once, extracting relevant variables per location type")
...
def main():
    """Example usage of the optimized specific points extraction."""
