...
warnings.filterwarnings(
    "ignore", category=DeprecationWarning, message=".*cryptography.*"
)
...
def get_wind_data_lat_long_cached(dt, directory, hours_forecasted="0"):
    ...
    try:
        ...
        valid_files = [
            f
            for f in grib_files
            if "subset_" not in os.path.basename(f) and not f.endswith(".idx")
        ]

        if not valid_files:
            print(
                f"No valid GRIB files found in {search_dir} (only subset/index files available)"
            )
            return None
...
def group_grib_files_by_time(files):
    ...
    for file in files:
        ...
        match = re.search(
            r"hrrr\.t(\d{2})z\.wrfsubhf(\d{2})\.grib2\.\w+\.idx", filename
        )
        if match:
            # Skip index files
            continue
...
def process_single_grib_file_single_pass(
    file_path,
    wind_selectors,
    solar_selectors,
    wind_indices,
    ...
    try:
        ...
        with pygrib.open(file_path) as grbs:
                    ...
                    if grb_name == var_selector:
                        ...
                        for offset, minute in [(0, 0), (15, 15), (30, 30), (45, 45)]:
                            if offset == 0:
                                ...
                                if (
                                    "0 mins" in grb_str
                                    or "0m mins" in grb_str
                                    or "mins" not in grb_str
                                ):
                        ...
                        for offset, minute in time_offsets_found:
                            ...
                            if var_name in wind_selectors and wind_indices is not None:
                                ...
                                wind_values[var_name][dt] = np.array(
                                    wind_extracted, dtype=np.float32
                                )
                            ...
                            if (
                                var_name in solar_selectors
                                and solar_indices is not None
                            ):
                                solar_extracted = []
                                ...
                                solar_values[var_name][dt] = np.array(
                                    solar_extracted, dtype=np.float32
                                )
            ...
            found_var_names = {
                var_name
                for var_name, timestamps in found_variables.items()
                if timestamps
            }
            missing_vars = set(all_selectors.keys()) - found_var_names
            if missing_vars:
                print(
                    f"  Missing variables in {os.path.basename(file_path)}: {missing_vars}"
                )
...
def process_single_day_single_pass(
    day_date,
    wind_locations,
    solar_locations,
    data_dir,
    ...
    if len(valid_files) >= 40:  # If we have at least 40 files, that's good
        ...
        print(
            f"⚠️  Missing {len(missing_times)} time groups (out of {len(expected_times)} expected)"
        )
        print(f"📊 Found {len(file_groups)} time groups")
    else:
        print(
            f"📊 Found {len(file_groups)} time groups (missing {len(missing_times)} minor gaps)"
        )
    ...
    sorted_time_groups = sorted(
        file_groups.items(),
        key=lambda x: (int(x[0].split("_")[0]), int(x[0].split("_")[1])),
    )

    print(
        f"⏰ Processing {len(sorted_time_groups)} time groups in chronological order..."
    )
    ...
    for time_key, files in sorted_time_groups:
        ...
        for file_path in files:
            ...
            wind_values, solar_values, file_timestamps = (
                process_single_grib_file_single_pass(
                    file_path,
                    wind_selectors,
                    solar_selectors,
                    wind_indices,
    ...
    for timestamp in all_timestamps:
        ...
        for var_name in wind_selectors.keys():
            if var_name in wind_data and timestamp in wind_data[var_name]:
                ...
                wind_data_final[var_name].append(
                    np.full(len(wind_indices) if wind_indices else 0, np.nan)
                )
        ...
        for var_name in solar_selectors.keys():
            if var_name in solar_data and timestamp in solar_data[var_name]:
                ...
                # Fill with NaN if no data for this timestamp
                solar_data_final[var_name].append(
                    np.full(len(solar_indices) if solar_indices else 0, np.nan)
                )
    ...
    try:
            ...
            for var_name, values_list in wind_data_final.items():
                if values_list:
                    ...
                    output_file = os.path.join(
                        wind_dirs[var_name], f"{day_date.strftime('%Y%m%d')}.parquet"
                    )
        ...
        if solar_data_final and solar_indices is not None:
            for var_name, values_list in solar_data_final.items():
                if values_list:
                    # Create DataFrame
                    df = pd.DataFrame(
                        values_list, columns=solar_locations["pid"].values
                    )
                    df.index = timestamps_final[: len(df)]

                    # Save to parquet
                    output_file = os.path.join(
                        solar_dirs[var_name], f"{day_date.strftime('%Y%m%d')}.parquet"
                    )
                    df.to_parquet(output_file, compression=compression)
        ...
        if wind_data_final and wind_indices is not None:
            ...
            if "UWind80" in wind_data_final and "VWind80" in wind_data_final:
                uwind80_df = pd.DataFrame(
                    wind_data_final["UWind80"], columns=wind_locations["pid"].values
                )
                vwind80_df = pd.DataFrame(
                    wind_data_final["VWind80"], columns=wind_locations["pid"].values
                )
                ...
                output_file = os.path.join(
                    wind_speed_dir, f"{day_date.strftime('%Y%m%d')}.parquet"
                )
        ...
        if solar_data_final and solar_indices is not None:
            ...
            if "UWind10" in solar_data_final and "VWind10" in solar_data_final:
                uwind10_df = pd.DataFrame(
                    solar_data_final["UWind10"], columns=solar_locations["pid"].values
                )
                vwind10_df = pd.DataFrame(
                    solar_data_final["VWind10"], columns=solar_locations["pid"].values
                )
                ...
                output_file = os.path.join(
                    solar_wind_speed_dir, f"{day_date.strftime('%Y%m%d')}.parquet"
                )
...
def extract_specific_points_daily_single_pass(
    ...
    print(
        f"📅 Processing {len(date_range)} days: {START.strftime('%Y-%m-%d')} to {END.strftime('%Y-%m-%d')}"
    )
    ...
    if PSUTIL_AVAILABLE:
        ...
        print(
            f"🖥️  System Resources: {cpu_count} CPUs, {memory.total / (1024**3):.1f}GB RAM"
        )
    else:
    ...
    if enable_resume:
        ...
        if completed_days:
            ...
            date_range = [
                d for d in date_range if d.strftime("%Y%m%d") not in completed_days
            ]
                ...
                try:
                    ...
                    for day_date in batch_dates:
                        try:
                            ...
                            print(
                                f"❌ Error processing {day_date.strftime('%Y-%m-%d')}: {day_error}"
                            )
    ...
    if successful_days:
        avg_time = sum(r["processing_time"] for r in successful_days) / len(
            successful_days
        )
